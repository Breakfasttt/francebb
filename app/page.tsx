      ...
      <div className="home-page-fixed">
        <section className="hero">
          ...
          <div className="action-grid-row top-row">
            <PremiumCard as={Link} href="/jouer" hoverEffect className="action-card primary-action">
              <Info size={28} className="icon-accent" />
              <div className="card-text large">Jouer à<br />Blood Bowl !</div>
            </PremiumCard>

            <PremiumCard as={Link} href="/forum" hoverEffect className="action-card">
              <MessageSquare size={24} className="icon-accent" />
              <div className="card-text">Forum</div>
            </PremiumCard>

            <div className="card-stack">
              <PremiumCard as={Link} href="/ligues" hoverEffect className="action-card flex-1">
                <Shield size={24} className="icon-accent" />
                <div className="card-text">Ligues</div>
              </PremiumCard>
              <PremiumCard as={Link} href={isAuth ? "/ligues/create" : "/auth/login?callback=/ligues/create"} hoverEffect className="action-card-mini">
                + Créer
              </PremiumCard>
            </div>

            <div className="card-stack">
              <PremiumCard as={Link} href="/tournaments" hoverEffect className="action-card flex-1">
                <Trophy size={24} className="icon-accent" />
                <div className="card-text">Tournois</div>
              </PremiumCard>
              {tournamentForum && (
                <PremiumCard as={Link} href={isAuth ? `/forum/new-tournament?forumId=${tournamentForum.id}` : `/auth/login?callback=/forum/new-tournament?forumId=${tournamentForum.id}`} hoverEffect className="action-card-mini">
                  + Créer
                </PremiumCard>
              )}
            </div>

            <PremiumCard as={Link} href="/carte" hoverEffect className="action-card">
          ...
          <div className="action-grid-row bottom-row">
            <PremiumCard as={Link} href="/classement" hoverEffect className="action-card">
              <Award size={24} className="icon-accent" />
              <div className="card-text">Coupe de France</div>
            </PremiumCard>
            <PremiumCard as={Link} href="/articles" hoverEffect className="action-card">
              <FileText size={24} className="icon-accent" />
              <div className="card-text">Articles</div>
            </PremiumCard>
            <PremiumCard as={Link} href="/ressources" hoverEffect className="action-card">
